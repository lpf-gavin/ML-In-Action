...
def createTree(dataSet, minSup=1): #create FP-tree from dataset but don't mine
    ...
    for k in headerTable.keys():  #remove items not meeting minSup
        if headerTable[k] >= minSup:
            newheaderTable[k] = headerTable[k]
...
def mineTree(inTree, headerTable, minSup, preFix, freqItemList):
    bigL = [v[0] for v in sorted(headerTable.items(), key=lambda x: x[1])] #(sort header table)
    for basePat in bigL:  #start from bottom of header table
        newPreFix = preFix.copy()
        newPreFix.add(basePat)
        #print 'finalFrequent Item: ',newFreqSet    #append to set
        freqItemList.append(newPreFix) #a result found
        ...
        if myHead != None: #3. mine cond. FP-tree
            ...
            #myCondTree.disp(1)            
            mineTree(myCondTree, myHead, minSup, newPreFix, freqItemList)
...
if __name__ == '__main__':
    ...
    print('initset', initSet, 'end initset')
